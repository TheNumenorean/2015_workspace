...
//		}
    ...
    protected double caculateVotageRampRate(double wheelSpeeds[]){
    	...
    	int timesAveraged = 0;
        for(int i = 0; i<NUMBER_OF_MOTORS; i++){
        	if(Math.abs(wheelSpeeds[i])>(Math.abs(this.motors[i].getOutputVoltage())/12)){
        	...
        	if(Math.abs(wheelSpeeds[i])<(Math.abs(this.motors[i].getOutputVoltage())/12)){
        		voltageRampRateAverage = ((voltageRampRateAverage * timesAveraged) + this.voltageRampRates[VOLTAGE_RAMP_RATE_DOWN_INDEX]*3)/(timesAveraged+3);
        		timesAveraged+=3;
        	}
        }
